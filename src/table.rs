...
impl Table {
  ...
  fn insert_leaf_node_for_leaf_root(&mut self,
                                    leaf_prev: NodeRc,
                                    leaf_new: NodeRc)
                                    -> Result<(), ExecErr> {
    ...
    leaf_prev.set_with(|nd| {
               let parent_new = root_new.new_parent_from_self();
               let next_leaf = NextLeaf { page: leaf_new.get_page_idx() as u32,
  ...
  pub fn insert_leaf_node(&mut self, leaf_prev: NodeRc, leaf_new: NodeRc) -> Result<(), ExecErr> {
    ...
    leaf_new.set_with(|nd| {
              let parent = Parent { page: parent.get_page_idx() as u32,
    ...
    if node.is_leaf() {
      ...
      node.set_with(|nd| {
            nd.to_intern_mut().set_child_by_key_with(key, |ch| {
                                if ch.node.is_none() {
                                  let n =
                                    self.load_node(ch.page as usize, intern_page_idx).unwrap();
  ...
  fn load_node(&self, page_idx: usize, parent_page: usize) -> Result<NodeRc, ExecErr> {
    ...
    node.set_with(|nd| {
      ...
      let parent = Parent { page: parent_page as u32,
                            node:
                              NodeRc::downgrade(self.pager.borrow().pages[parent_page].as_ref()
                                                                                      .unwrap()) };
      nd.set_parent(parent)
    });
    Ok(node)
  ...
  fn btree_to_str_rec(&self, node: &NodeRc) -> String {
    ...
    let s = node.set_with(|intern| {
                  ...
                  for Child { page, node, .. } in &mut intern.to_intern_mut().children {
                    ...
                    let s: String = self.btree_to_str_rec(node.as_ref().unwrap())
                                        .lines()
                                        .map(|s| format!("  {}\n", s))
                                        .collect();
                    string.push_str(&s);
