use crate::btree::intern2::{Child, Intern};
use crate::btree::leaf2::Leaf;
use crate::btree::node2::Node;
use crate::cursor2::Cursor;
...
impl Table {
  ...
  pub fn insert_row(&mut self, key: u32, row: &Row) -> Result<(), ExecErr> {
    ...
    let leaf_idx = self.find_leaf_recurs(0, key)?;
    let res = self
      .pager
    ...
    match res {
      Err(ExecErr::LeafNodeFull(_)) => {
        ...
        let child = Child::new(pg_idx_new, Some(key_max));
    ...
    match parent {
      ...
      Some(pg) => {
        let res = self.pager.set_node_by(pg, |nd| {
          nd.as_intern_mut()
            .unwrap()
            .insert_child(child.pg_idx, child.key_max.unwrap_or(0))
        })?;
        match res {
          Err(ExecErr::InternNodeFull(_)) => {
            let intern = self.pager.set_node_by(pg, |nd| {
              nd.as_intern_mut()
                .unwrap()
                .insert_child_and_split(child.pg_idx, child.key_max.unwrap_or(0))
            })??;
            let pg_idx_new = self.pager.size();
  ...
  fn new_root_and_insert_child(&mut self, child_rht: Child) -> Result<(), ExecErr> {
    ...
    let root_new = self.pager.set_node_by(ROOT, |nd| {
      ...
      nd.set_parent(Some(ROOT));
      let child_lft = Child::new(pg_idx_new, Some(key_max));
